...
class HedgedCopulaPairsTrading(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.Schedule.On(
            ...
            self.UpdateHMMRegime
                ...
                # === VaR settings ===
        self.var_conf_levels = [0.95, 0.99]  
        self.var_lookback = 252               
        self._tpv_window = deque(maxlen=self.var_lookback + 1)
        self._ret_window = deque(maxlen=self.var_lookback)

        self.Schedule.On(
            self.DateRules.EveryDay(self.spy),
            self.TimeRules.AfterMarketClose(self.spy, 0),
            self._RecordTpvAndComputeVaR
        )
    ...
    def _RecordTpvAndComputeVaR(self):
        tpv_today = float(self.Portfolio.TotalPortfolioValue)
        if not math.isfinite(tpv_today):
            return


        if len(self._tpv_window) > 0:
            tpv_prev = self._tpv_window[-1]
            if tpv_prev > 0:
                r = tpv_today / tpv_prev - 1.0
                if math.isfinite(r):
                    self._ret_window.append(r)
        self._tpv_window.append(tpv_today)

        if len(self._ret_window) < max(60, int(self.var_lookback * 0.4)):
            return

        rets = list(self._ret_window)
        rets.sort()

        for conf in self.var_conf_levels:

            q = self._percentile(rets, 1.0 - conf)
            var_amt = max(0.0, -q * tpv_today)              
            cvar_amt = self._cvar_amount(rets, q, tpv_today)


            label = f"VaR{int(conf*100)}"
            self.Plot("VaR", label, var_amt)
            self.Plot("VaR", f"CVaR{int(conf*100)}", cvar_amt)

            self.SetRuntimeStatistic(f"{label} (USD)", f"{var_amt:,.0f}")
            self.SetRuntimeStatistic(f"CVaR{int(conf*100)} (USD)", f"{cvar_amt:,.0f}")


        if len(self._ret_window) > 0:
            self.Plot("VaR", "DailyPnL", self._ret_window[-1] * tpv_today)
    ...
    def _cvar_amount(self, sorted_rets, q_ret, tpv):
        tail = [r for r in sorted_rets if r <= q_ret]
        if not tail:
            return 0.0
        avg_loss_ret = sum(tail) / len(tail)  # 负数
        return max(0.0, -avg_loss_ret * tpv)
    ...
    def OnEndOfAlgorithm(self):
        ...
        rets = list(self._ret_window); rets.sort()
        for conf in self.var_conf_levels:
            q = self._percentile(rets, 1.0 - conf)
            var_amt = max(0.0, -q * tpv)
            cvar_amt = self._cvar_amount(rets, q, tpv)
            self.Debug(f"[VaR] {int(conf*100)}% VaR={var_amt:,.2f}  CVaR={cvar_amt:,.2f}  (TPV={tpv:,.2f})")
