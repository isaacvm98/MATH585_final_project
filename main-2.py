...
class HedgedCopulaPairsTrading(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # VaR / CVaR Plot Setup
        # -----------------------------
        self.var_lookback = 252          # Rolling window size (number of trading days)
        self.var_min_samples = 50        # Minimum samples required before computing VaR/CVaR
        self.pnl_window = deque(maxlen=self.var_lookback)
        self.prev_portfolio_value = None  # Used to compute daily PnL from portfolio value changes

        # Create a chart named "VaR" and add the series you want to plot
        var_chart = Chart("VaR")
        var_chart.AddSeries(Series("DailyPnL", SeriesType.Line, 0))
        var_chart.AddSeries(Series("VaR95",   SeriesType.Line, 0))
        var_chart.AddSeries(Series("CVaR95",  SeriesType.Line, 0))
        var_chart.AddSeries(Series("VaR99",   SeriesType.Line, 0))
        var_chart.AddSeries(Series("CVaR99",  SeriesType.Line, 0))
        self.AddChart(var_chart)

        # Compute and plot VaR/CVaR once per day after market close
        self.Schedule.On(
            self.DateRules.EveryDay(self.spy),
            self.TimeRules.AfterMarketClose(self.spy, 0),
            self.UpdateVaRPlot
        )

        self.SetWarmUp(253)

    def UpdateVaRPlot(self):
        """Compute daily PnL and rolling VaR/CVaR, then plot them."""
        if self.IsWarmingUp:
            return
        ...
        # Initialize previous value on the first run
        if self.prev_portfolio_value is None:
            self.prev_portfolio_value = tpv
            return

        # Daily PnL in dollars (difference in total portfolio value from yesterday to today)
        daily_pnl = tpv - self.prev_portfolio_value
        self.prev_portfolio_value = tpv

        # Update rolling window
        self.pnl_window.append(daily_pnl)

        # Always plot DailyPnL
        self.Plot("VaR", "DailyPnL", daily_pnl)

        # Not enough samples yet -> skip VaR/CVaR calculation
        if len(self.pnl_window) < self.var_min_samples:
            return

        pnl = np.asarray(self.pnl_window, dtype=float)

        # VaR is defined as the left-tail percentile of the PnL distribution.
        # VaR95 = 5th percentile, VaR99 = 1st percentile (usually negative numbers).
        var95 = float(np.percentile(pnl, 5))
        var99 = float(np.percentile(pnl, 1))

        # CVaR (Expected Shortfall) is the average PnL in the tail beyond VaR
        tail95 = pnl[pnl <= var95]
        tail99 = pnl[pnl <= var99]

        cvar95 = float(tail95.mean()) if len(tail95) > 0 else var95
        cvar99 = float(tail99.mean()) if len(tail99) > 0 else var99

        # Plot VaR/CVaR series
        self.Plot("VaR", "VaR95",  var95)
        self.Plot("VaR", "CVaR95", cvar95)
        self.Plot("VaR", "VaR99",  var99)
        self.Plot("VaR", "CVaR99", cvar99)
